...
const CategoryPage = () => {
  ...
  const [selectedDiscount, setSelectedDiscount] = useState(0); // Filter for discount >= x
  const [selectedTags, setSelectedTags] = useState([]); // Filter based on tags
  ...
  // Apply filters based on color, discount, and tags
  useEffect(() => {
    ...
    // Filter by Discount (>= selected discount)
    if (selectedDiscount) {
      filtered = filtered.filter((product) => product.discount >= selectedDiscount);
    }
  ...
  }, [allProducts, selectedColor, selectedDiscount, selectedTags]);
      ...
      <div className="row">
        {loading ? (
          ...
          paginatedProducts.map((product) => {
            // Check if the product has necessary fields
                  ...
                  <div className="card border-0 rounded-3 simple-box">
                    <Link to={`product/${product.id}`}>
                      <div className="p-1 position-relative soft-blur">
                        ...
                        />
                        <div
                          className={`badge soft-blur position-absolute top-0 start-50 ms-3 mt-2 ${getTagColor(product.tags)}`}
                        >
                          {product.tags}
                        </div>
                      </div>
